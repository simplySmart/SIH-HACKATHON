...
// regex to find the conditional response block
// {incident.status !== 'detected' && (\s*<div className="bg-\[#1C2721\].*?</div>\s*</div>\s*</div>\s*\)}
// Using simpler replace between "</div>\s*</div>\s*</div>\s*</div>\s*)}\s*{/* Evidence Sources */}"
const startStr = "{incident.status !== 'detected' && (";
