  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex items-center justify-center w-8 h-8">
        ${isCritical ? `<div class="absolute -inset-4 bg-red-500/20 rounded-full animate-pulse"></div>
          ...
          <div className="flex flex-wrap items-center gap-2 md:gap-4 w-full md:w-auto">
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 ml-0 md:ml-2">
              ...
              {config.DEMO_MODE ? (
                ...
                <span className="bg-blue-500/20 text-blue-300 text-[10px] px-2.5 py-1 rounded-md font-bold uppercase tracking-wider border border-blue-500/30 flex items-center gap-1.5">
      ...
      <div className="flex-1 flex flex-col lg:flex-row gap-6 overflow-hidden relative">
        ...
        <div className="hidden lg:flex w-72 flex-col gap-6 overflow-y-auto pb-4 shrink-0">
          ...
          <div className="bg-gradient-to-br from-[#111c16] to-[#0a120d] rounded-3xl p-6 text-white shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/5 relative overflow-hidden">
            ...
            <h3 className="text-sm font-semibold text-gray-300 mb-5 tracking-wide uppercase">{selectedIncident ? 'Incident Weather' : 'Regional Telemetry'}</h3>
            {(() => {
          ...
          {/* Filters */}
          <div className="bg-[#0f1912]/80 backdrop-blur-3xl rounded-3xl p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-white/10 flex flex-col">
            <h3 className="text-sm font-bold text-white mb-4">Map Controls</h3>
            
            <div className="mb-4">
              <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Search District</label>
              <div className="relative">
                ...
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-300">
                    <X className="w-4 h-4" />
                  </button>
                )}
              </div>
                    ...
                    >
                      <Popup className="custom-popup" closeButton={false}>
                        <div className="p-1 min-w-[240px]">
                          <div className="text-xs text-gray-400 font-semibold mb-1 uppercase tracking-wider">{incident.id}</div>
                          <h3 className="font-bold text-white text-base leading-tight mb-1">{incident.title}</h3>
                          
                          <div className="flex flex-col gap-1 text-xs mb-3 text-gray-300">
                            ...
                            {incident.detection.forestScreening && (
                                <div><span className="font-semibold">Screening:</span> <span className={`font-medium ${incident.detection.forestScreening === 'FOREST' ? 'text-green-400' : incident.detection.forestScreening === 'NEAR FOREST' ? 'text-yellow-600' : 'text-gray-400'}`}>{incident.detection.forestScreening}</span></div>
                            )}
              ...
              <div className="bg-[#121E15]/95 backdrop-blur-sm border border-white/10 rounded-xl px-4 py-3 flex flex-col gap-2 shadow-lg">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-200 text-xs font-medium">Low</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-yellow-500"></div>
                    <span className="text-gray-200 text-xs font-medium">Moderate</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-3 h-3 rounded-full bg-orange-500"></div>
                    <span className="text-gray-200 text-xs font-medium">High</span>
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-gray-200 text-xs font-medium ml-1">Critical</span>
                  </div>
                </div>
        ...
        {selectedIncident && (
           <div className="w-full lg:w-80 fixed lg:relative bottom-[calc(env(safe-area-inset-bottom)+60px)] lg:bottom-auto left-0 right-0 z-[2000] lg:z-auto bg-[#0f1912]/95 lg:bg-[#0f1912]/90 backdrop-blur-3xl rounded-t-3xl lg:rounded-3xl p-6 shadow-[0_-10px_40px_rgb(0,0,0,0.2)] lg:shadow-[0_12px_40px_rgb(0,0,0,0.08)] border-t lg:border border-white/10 overflow-y-auto max-h-[50vh] lg:max-h-none lg:shrink-0 flex flex-col">
             ...
             <div className="mb-6 pr-8">
               ...
               <h3 className="text-xl font-bold text-white leading-tight mb-2">{selectedIncident.title}</h3>
               <p className="text-sm text-gray-400">{selectedIncident.location.district}, {selectedIncident.location.state}</p>
             </div>

             <div className="space-y-4 mb-6 flex-1">
               <div className="flex justify-between items-center py-2 border-b border-white/10">
                 <span className="text-sm text-gray-400">Coordinates</span>
                 <span className="text-sm font-medium text-white">{selectedIncident.location.coordinates.lat.toFixed(4)}, {selectedIncident.location.coordinates.lng.toFixed(4)}</span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-white/10">
                 <span className="text-sm text-gray-400">Forest Screening</span>
                 <span className={`text-sm font-medium ${selectedIncident.detection.forestScreening === 'FOREST' ? 'text-green-400' : selectedIncident.detection.forestScreening === 'NEAR FOREST' ? 'text-yellow-600' : 'text-gray-400'}`}>
                   {selectedIncident.detection.forestScreening || 'UNKNOWN'}
                 </span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-white/10">
                 <span className="text-sm text-gray-400">Satellite</span>
                 <span className="text-sm font-medium text-white">{selectedIncident.title.replace('Satellite Detection (', '').replace(')', '')}</span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-white/10">
               ...
               <div className="flex justify-between items-center py-2 border-b border-white/10">
                 ...
                 <span className="text-sm font-medium text-white">{selectedIncident.detection.confidence}%</span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-white/10">
                 <span className="text-sm text-gray-400">FRP (MW)</span>
                 <span className="text-sm font-medium text-white">{selectedIncident.impact.areaAffectedHa}</span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-white/10">
                 <span className="text-sm text-gray-400">Time Detected</span>
                 <span className="text-sm font-medium text-white">{new Date(selectedIncident.detection.time).toLocaleString([], {hour: '2-digit', minute:'2-digit', day: '2-digit', month: 'short', year: 'numeric'})}</span>
               </div>
               <div className="flex justify-between items-center py-2 border-b border-white/10">
                 <span className="text-sm text-gray-400">Data Source</span>
                 <span className="text-sm font-medium text-white">NASA FIRMS</span>
               </div>
             </div>
